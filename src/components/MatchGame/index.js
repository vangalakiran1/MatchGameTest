...
import SmallImgItem from '../SmallImgItem'
import './index.css'
...
class MatchGame extends Component {
  ...
  matchImgCheck = id => {
    ...
    const checkImg = imagesList.filter(eachVal => {
      if (eachVal.id === id) {
        return eachVal
      }
      return ''
    })
    const {imageUrl} = checkImg[0]
    if (imageUrl === imgURL) {
      const randomNum = Math.floor(Math.random() * (imagesList.length - 1))
      const randomUrl = imagesList[randomNum].imageUrl
  ...
  render() {
    const {tabId, timerSeconds, isTrue, score, imgURL} = this.state
            ...
            <ul className="score-box">
              ...
              <li>
                <img
                  ...
                  className="timer-img"
                />
                <p className="text-color"> {timerSeconds} sec</p>
              </li>
            </ul>
          </nav>
        </div>
              ...
              <ul className="tabs-box">
                {tabsList.map(eachTab => (
                  <TabItem
                    ...
                    isActiveTrue={tabId === eachTab.tabId}
                  />
                ))}
              </ul>
            </div>

            <div className="thumbnails-img-box">
              <ul className="item-box-container">
                {filteredList.map(eachItem => (
                  <SmallImgItem
                    key={eachItem.id}
                    imgDetails={eachItem}
                    onClickImg={this.matchImgCheck}
                  />
                ))}
              </ul>
            </div>
